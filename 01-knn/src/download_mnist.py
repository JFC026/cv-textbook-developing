...
下载并预处理 MNIST 数据集，并把训练/测试数据保存为 .npy 文件，供后续 K-NN 或基于 NumPy 的实验使用。

说明:
- 该脚本会使用 sklearn.datasets.fetch_openml 获取 'mnist_784' 数据集（可能需要网络）。
- 数据将被归一化到 [0,1]，并按近似 60000/10000 的比例划分训练/测试集（使用自定义的 train_test_split_numpy）。
- 最终保存为: data/X_train.npy, data/y_train.npy, data/X_test.npy, data/y_test.npy
"""
